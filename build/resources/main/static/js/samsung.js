...
async function registerProduct() {
    ...
    const response = await fetch('/db/samsung/products');
    if (!response.ok) {
    ...
    confirmButton.onclick = async () => {
        const saveResponse = await fetch('/db/samsung', {
            method: 'POST',
...
async function loadProducts() {
    const response = await fetch('/db/samsung/products'); // 서버에서 상품 목록 가져오기
    if (!response.ok) {
...
async function deleteProduct(productNo) {
    ...
    try{
        console.log(`Requesting DELETE for: /db/samsung/${productNo}`);
        const response = await fetch(`/db/samsung/${productNo}`, {
            method: 'DELETE',
        });

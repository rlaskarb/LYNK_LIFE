...
async function registerProduct() {
    ...
    const response = await fetch('/db/metlife/products');
    if (!response.ok) {
    ...
    confirmButton.onclick = async () => {
        const saveResponse = await fetch('/db/metlife', {
            method: 'POST',
...
async function loadProducts() {
    const response = await fetch('/db/metlife/products'); // 서버에서 상품 목록 가져오기
    if (!response.ok) {
...
async function deleteProduct(productNo) {
    ...
    try{
        console.log(`Requesting DELETE for: /db/metlife/${productNo}`);
        const response = await fetch(`/db/metlife/${productNo}`, {
            method: 'DELETE',
        });

...
async function registerProduct() {
    ...
    const response = await fetch('/db/hanwhalife/products');
    if (!response.ok) {
    ...
    confirmButton.onclick = async () => {
        const saveResponse = await fetch('/db/hanwhalife', {
            method: 'POST',
...
async function loadProducts() {
    const response = await fetch('/db/hanwhalife/products'); // 서버에서 상품 목록 가져오기
    if (!response.ok) {
...
async function deleteProduct(productNo) {
    ...
    try{
        console.log(`Requesting DELETE for: /db/hanwhalife/${productNo}`);
        const response = await fetch(`/db/hanwhalife/${productNo}`, {
            method: 'DELETE',
        });

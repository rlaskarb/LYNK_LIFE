...
async function registerProduct() {
    ...
    const response = await fetch('/db/shinhan/products');
    if (!response.ok) {
    ...
    confirmButton.onclick = async () => {
        const saveResponse = await fetch('/db/shinhan', {
            method: 'POST',
...
async function loadProducts() {
    const response = await fetch('/db/shinhan/products'); // 서버에서 상품 목록 가져오기
    if (!response.ok) {
...
async function deleteProduct(productNo) {
    ...
    try{
        console.log(`Requesting DELETE for: /db/shinhan/${productNo}`);
        const response = await fetch(`/db/shinhan/${productNo}`, {
            method: 'DELETE',
        });
